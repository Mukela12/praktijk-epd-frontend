...
const testUsers = {
  admin: {
    ...
    password: 'Milan18$',
  twoFactorCode: '358452' // Replace with fresh 2FA code before running
  },
  therapist: {
    email: 'codelibrary21@gmail.com',
    password: 'Milan18$',
    twoFactorCode: '202375' // Replace with fresh 2FA code before running
  },
  client: {
...
// Main test runner
async function testNewMissingEndpoints() {
  console.log(`\n${colors.cyan(colors.bold('🆕 Testing New Missing Endpoints'))}`);
  
  // Test Update Session Progress
  if (testSessionId && therapistToken) {
    const updateProgress = await apiRequest('PUT', `/sessions/${testSessionId}/progress`, {
      progressNotes: 'Client is showing improvement in managing anxiety',
      goalsDiscussed: 'Reviewed breathing techniques, introduced grounding exercises',
      clientMoodCurrent: 6,
      techniquesUsed: ['CBT', 'Grounding', 'Breathing Exercises'],
      interventionsApplied: ['Cognitive Restructuring', 'Relaxation Training']
    }, therapistToken);
    logTestResult('Update session progress', updateProgress);
  }

  // Test Client Session Summary
  if (testClientId && therapistToken) {
    const sessionSummary = await apiRequest('GET', `/therapist/clients/${testClientId}/session-summary`, null, therapistToken);
    logTestResult('Get client session summary', sessionSummary);
    
    if (sessionSummary.success) {
      console.log(colors.gray(`   Total sessions: ${sessionSummary.data?.data?.totalSessions || 0}`));
      console.log(colors.gray(`   Average mood improvement: ${sessionSummary.data?.data?.averageMoodImprovement || 'N/A'}`));
    }
  }

  // Test Client Medical History
  if (testClientId && therapistToken) {
    const medicalHistory = await apiRequest('GET', `/therapist/clients/${testClientId}/medical-history`, null, therapistToken);
    logTestResult('Get client medical history', medicalHistory);
  }

  // Test Client Therapy Goals
  if (testClientId && therapistToken) {
    const therapyGoals = await apiRequest('GET', `/therapist/clients/${testClientId}/therapy-goals`, null, therapistToken);
    logTestResult('Get client therapy goals', therapyGoals);
    
    if (therapyGoals.success) {
      console.log(colors.gray(`   Active goals: ${therapyGoals.data?.data?.activeGoals || 0}`));
    }
  }

  // Test No-Show Policy Application
  if (testAppointmentId && therapistToken) {
    // First create a test appointment that we can mark as no-show
    const testDate = new Date();
    testDate.setDate(testDate.getDate() - 1); // Yesterday
    
    const noShowAppointment = await apiRequest('POST', '/therapist/appointments', {
      clientId: testClientId,
      ...
      therapyType: 'individual',
      location: 'office',
      notes: 'Test appointment for no-show policy'
    }, therapistToken);
    
    if (noShowAppointment.success) {
      const apptId = noShowAppointment.data.data.id;
      
      const applyNoShow = await apiRequest('POST', `/therapist/appointments/${apptId}/no-show-policy`, {
        reason: 'Client did not attend without notice',
        applyFee: true,
        feeAmount: 50.00,
        notifyClient: true
      }, therapistToken);
      
      logTestResult('Apply no-show policy', applyNoShow);
    }
  }

  // Test Session Statistics
  if (therapistToken) {
    const sessionStats = await apiRequest('GET', `/therapist/${testTherapistId}/session-stats`, {
      startDate: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
      endDate: new Date().toISOString().split('T')[0]
    }, therapistToken);
    
    logTestResult('Get therapist session statistics', sessionStats);
    
    if (sessionStats.success) {
      const stats = sessionStats.data?.data;
      console.log(colors.gray(`   Total sessions: ${stats?.totalSessions || 0}`));
      console.log(colors.gray(`   Client retention rate: ${stats?.clientRetentionRate || 0}%`));
    }
  }
}
